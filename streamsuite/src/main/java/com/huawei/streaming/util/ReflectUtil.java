...
public class ReflectUtil
{
    ...
    private Method getSimilarMethod(String name, Class< ? >[] types) throws ReflectiveOperationException
    {
        ...
        while (type != null);
        throw new ReflectiveOperationException("No similar method " + name + " with params " + Arrays.toString(types) + " could be found on type " + type() + ".");
    }

    private boolean isSimilarSignature(Method possiblyMatchingMethod, String desiredMethodName,
     Class< ? >[] desiredParamTypes)
    {
        return possiblyMatchingMethod.getName().equals(desiredMethodName) && isMatch(possiblyMatchingMethod.getParameterTypes(), desiredParamTypes);
    }
