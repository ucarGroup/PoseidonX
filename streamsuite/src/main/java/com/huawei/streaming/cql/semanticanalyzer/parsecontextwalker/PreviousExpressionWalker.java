...
public class PreviousExpressionWalker implements ParseContextWalker
{
    ...
    public boolean walk(ParseContext parseContext)
    {
        ...
        {
            ExpressionPreviousContext previousContext = (ExpressionPreviousContext)parseContext;
            
            if (isMultiPrevious(previousContext))
            {
                isHasMultiPreviousExpression = true;
            }
            return true;
        }
        return false;
    ...
    private boolean isMultiPrevious(ExpressionPreviousContext previousContext)
    {
        return previousContext.getExpressions().size() > CQLConst.I_2;
    }
    
    public boolean isHasMultiPreviousExpression()
    {
        return isHasMultiPreviousExpression;
    }
}
